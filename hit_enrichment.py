...
def hit_enrichment(dataframe, prediction_column="proba1"):
    ...
    dataframe.sort_values(by=prediction_column, axis=0, ascending=False, inplace=True)
    dataframe.reset_index(drop=True, inplace=True)
    
    #Count molecules
    total_mols = len(dataframe)
    active_mols = len(dataframe[dataframe["bin"]==1])
    ...
    hit_enr = [x/active_mols for x in np.cumsum(dataframe["bin"])]
    ...
    inactives_last = [active_mols]*(len(dataframe[dataframe["bin"]==0])) 	#for each inactive, the maximum
